...
export default function Teste4_2_2() {

  const navigation = useNavigation();

  const { teste, enviaResultadoBancoT4,marcador,marcarAluno} = useContext(AuthContext);

  var start = getRandomIntInclusive(1, 90);
  var vetor= [start,start+2,start+4,start+6,start+8,start+10,start+12,start+14] ;
  ...
  function opcao(resposta1, resposta2){//Avalia resposta e segue para o proximo
    ...
    enviaResultadoBancoT4();
    if(marcador == false){

    navigation.reset({
      index: 0,
      routes: [{name: 'EscolherAluno'}],
    });
    }else{
      console.log("chegou no marcado!!");
      marcarAluno(false);
      navigation.reset({
        index:0,
        routes: [{name: 'Resultados_1'}],
      });
    }
...
<Text style={styles.valor1}>{vetor[0]},{vetor[1]},{vetor[2]},{vetor[3]},{vetor[4]},
      ...
      <Animatable.View animation="zoomIn" style={styles.viewBtn}>
          ...
          <View style={styles.viewBtn3e4}>
            <TouchableOpacity style={styles.btn} onPress={() => opcao(possibilidadesRestantes[3],possibilidadesRestantes[4])}>
              <Text style={styles.btnNum}>{possibilidadesRestantes[3]} e {possibilidadesRestantes[4]}</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.btn} onPress={() => opcao(possibilidadesRestantes[2],possibilidadesRestantes[5])}>
              <Text style={styles.btnNum}>{possibilidadesRestantes[2]} e {possibilidadesRestantes[5]}</Text>
            </TouchableOpacity>
          </View>
      </Animatable.View>
